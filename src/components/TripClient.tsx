...
const fmt = (d: string) =>
  new Date(d.slice(0, 10) + "T00:00:00").toLocaleDateString(undefined, {
    month: "short",
    day: "numeric",
  });
...
export default function TripClient({ trip: initial }: Props) {
  ...
  const setTransitCaveatDismissed = useTripStore(
    (s) => s.setTransitCaveatDismissed,
  );
  ...
  const pendingCount = trip.locations.filter(
  ...
  const pathGeometry = usePathGeometry(
    trip.id,
    days,
    roadProfile,
      ...
      <div className={`space-y-6 ${discoveryMode ? "pb-56" : ""}`}>
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-body text-sub mt-0.5">
              <span className="text-numeral">{trip.locations.length}</span>{" "}
              location
              {trip.locations.length !== 1 ? "s" : ""} · {fmt(trip.startDate)} →{" "}
              {fmt(trip.endDate)} ·{" "}
              <span className="text-numeral">{numDays}</span> day
              {numDays !== 1 ? "s" : ""}
            </p>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {activeSurface === "itinerary" && hasPlan && (
            <button
              onClick={() => setMapPopupOpen(!mapPopupOpen)}
        ...
        {activeSurface === "places" ? (
          ...
          <div className="space-y-4">
            {showTransitCaveat && (
              <TransitEstimateCaveat
                onDismiss={() => setTransitCaveatDismissed(true)}
              />
            )}
            <DayNavigator />
          </div>
  ...
  return (
    <div className="card p-8 text-center text-sub space-y-2">
      ...
      <p className="text-sm">
        Add places under{" "}
        <button
          onClick={() => setActiveSurface("places")}
