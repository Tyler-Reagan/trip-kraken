    ...
    >
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="tap-target text-faint hover:text-ink"
            aria-label="Close"
          >
            <X className="w-4 h-4" />
          </button>
...
export function AssignExisting({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        onClick={() => locationId && onAssign(locationId)}
        disabled={!locationId}
        className="btn-primary text-xs py-1 px-3"
      >
        Assign
      </button>
      <button onClick={onCancel} className="btn-secondary text-xs py-1 px-3">
        Cancel
      </button>
    </div>
  );
...
function NightStripWizard({
  metros: liveMetros,
  onClose,
}: {
  metros: UncoveredMetro[];
  onClose: () => void;
}) {
  ...
  const nightBounds = trip
    ? {
        first: trip.startDate,
        last: trip.endDate,
        checkoutMax: addDaysIso(trip.endDate, 1),
      }
    : null;
  ...
  const outcomeOf = (key: string): StepOutcome | undefined =>
    outcomes[key] ?? (!liveKeys.has(key) ? "assigned" : undefined);
  ...
  async function handleManualSave() {
    ...
    if (!checkInDate || !checkOutDate) {
      setError("Add both a check-in and check-out date, or skip this city.");
      return;
    }
    setSaving(true);
    const err = await saveLodgingDates(locationId, {
      checkInDate,
      checkOutDate,
    });
    setSaving(false);
      ...
      {done ? (
        <div className="space-y-3">
            ...
            {sequence.map((key) => {
              ...
              return (
                <li key={key} className="flex items-start gap-2 text-sm">
                  <span
                    className={`w-3.5 shrink-0 mt-0.5 text-center ${assigned ? "text-brand-600 dark:text-brand-400" : "text-faint"}`}
                  >
                    {assigned ? "✓" : "–"}
                  </span>
          ...
          <p className="text-sm text-sub">
            Skipped areas stay unassigned and can be added anytime from the
            night strip above.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
                ...
                return (
                  <span
                    ...
                    className={`px-3 py-1.5 rounded-full flex items-center gap-1.5 transition-all ${
                      isCurrent
                        ? "bg-brand-600 text-white text-sm font-medium ring-2 ring-brand-500/30 scale-105"
                        : isHandled
                          ? "bg-surface-2 text-faint text-xs line-through"
                          : "bg-surface-2 text-sub text-xs border border-line-strong"
                    }`}
                  >
                    {metroLabel(m)}
                    <span
                      className={`text-numeral ${isCurrent ? "text-[11px]" : "text-[10px]"} opacity-90`}
                    >
                      {m.activities.length}
                    </span>
                  </span>
          ...
          <p className="text-sm text-sub">
            ...
            <span className="text-ink font-medium">
              {metroLabel(metro)}
            </span> — {metro.activities.length} imported place
            {metro.activities.length !== 1 ? "s" : ""} there
          </p>
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-faint">
              Paste a booking confirmation email
            </label>
            <textarea
              value={bookingText}
              ...
              placeholder={
                "Property: Shibuya Grand\nCheck-in: Aug 13, 2026\nCheck-out: Aug 17, 2026"
              }
            />
            <p className="text-[11px] text-faint">
              We&rsquo;ll pull the property name and dates out automatically.
            </p>
            <button
              onClick={handleImport}
              disabled={saving || !bookingText.trim()}
              className="btn-primary text-xs py-1.5 px-3 w-full disabled:opacity-40"
            >
              Import & continue
            </button>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-faint">
              Pick one of the places you already imported
            </label>
            <select
              value={locationId}
              onChange={(e) => setLocationId(e.target.value)}
            ...
            <div className="flex items-center gap-1.5">
              <input
                type="date"
                value={checkInDate}
                min={nightBounds?.first}
                max={nightBounds?.last}
              ...
              <input
                type="date"
                value={checkOutDate}
                min={checkInDate || nightBounds?.first}
                max={nightBounds?.checkoutMax}
            ...
            <button
              ...
              className="btn-secondary text-xs py-1.5 px-3 w-full disabled:opacity-40"
            >
              Save & continue
            </button>
          </div>
  ...
  return (
    <div className="card p-3 flex items-start gap-2 text-sm">
      ...
      <p className="flex-1 text-sub">
        This trip spans multiple days without any lodging — if you're staying
        somewhere overnight, drag across the nights above to add it.
      </p>
      <button
        onClick={onDismiss}
...
function laneBarsOf(
  ...
  for (const l of lodgings) {
    ...
    while (
      bars.some(
        (b) => b.lane === lane && !(endIdx < b.startIdx || startIdx > b.endIdx),
      )
    )
      lane++;
...
export function NightStrip({
  ...
  const metros = useMemo(
    () => clusterByMetro<Location, Lodging>(activities, lodgings),
    [activities, lodgings],
  );
  const uncoveredMetros = useMemo(
    () =>
      metros.filter((m) => m.lodgings.length === 0 && m.activities.length > 0),
    [metros],
  );
  ...
  const isSingleMetroNoLodging =
    metros.length === 1 && isMultiDay && lodgings.length === 0;

  const [wizardOpen, setWizardOpen] = useState(false);
  const [durationTipDismissed, setDurationTipDismissed] = useState(false);
  const [select, setSelect] = useState<{ start: number; end: number } | null>(
    null,
  );
  ...
  const [drag, setDrag] = useState<{
    ...
    anchorIdx: number;
    preview: { startIdx: number; endIdx: number };
  } | null>(null);
  ...
  useEffect(() => {
    ...
    if (!isSingleMetroNoLodging && uncoveredMetros.length > 0)
      setWizardOpen(true);
  ...
  function idxFromClientX(x: number) {
    ...
    return Math.min(
      nights.length - 1,
      Math.max(0, Math.floor(((x - rect.left) / rect.width) * nights.length)),
    );
  }
  ...
  function startSelect(i: number, e: React.PointerEvent) {
    ...
    const onUp = (ev: PointerEvent) => {
      ...
      setPendingAssign({
        startIdx: Math.min(i, end),
        endIdx: Math.max(i, end),
      });
      setSelect(null);
    };
  ...
  function startBarDrag(
    ...
    const previewRef = {
      current: { startIdx: bar.startIdx, endIdx: bar.endIdx },
    };
    setDrag({
      lodgingId: bar.lodging.id,
      mode,
      anchorIdx,
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      if (mode === "resize-start")
        next = { startIdx: Math.min(i, bar.endIdx), endIdx: bar.endIdx };
      else if (mode === "resize-end")
        next = { startIdx: bar.startIdx, endIdx: Math.max(i, bar.startIdx) };
      else {
        const delta = i - anchorIdx;
        const span = bar.endIdx - bar.startIdx;
        const start = Math.min(
          Math.max(0, bar.startIdx + delta),
          nights.length - 1 - span,
        );
    ...
    const onUp = (ev: PointerEvent) => {
      ...
      if (mode === "move" && unchanged) setEditing(bar.lodging.id);
        ...
        saveLodgingDates(bar.lodging.id, {
          checkInDate: nights[startIdx],
          checkOutDate: addDaysIso(nights[endIdx], 1),
        });
      setDrag(null);
    };
    window.addEventListener("pointermove", onMove);
  ...
  const selectRange = select
    ? {
        startIdx: Math.min(select.start, select.end),
        endIdx: Math.max(select.start, select.end),
      }
    : null;
  const pct = (idx: number) => `${(idx / nights.length) * 100}%`;
  const widthPct = (startIdx: number, endIdx: number) =>
    `${((endIdx - startIdx + 1) / nights.length) * 100}%`;
  ...
  return (
    <div className="space-y-1.5">
      <div
        ...
        style={{ height: laneCount * STRIP_LANE_H }}
      >
        {nights.map((date, i) => (
          <button
            key={date}
          ...
          >
            {!bars.some((b) => i >= b.startIdx && i <= b.endIdx) && (
              <span
                className="block mx-px rounded-sm bg-[repeating-linear-gradient(45deg,var(--surface-3),var(--surface-3)_3px,transparent_3px,transparent_6px)] border border-dashed border-line-strong"
                style={{ height: STRIP_LANE_H - 2 }}
              />
            )}
          </button>
        ))}
        ...
        {selectRange && (
          <div
            ...
            style={{
              left: pct(selectRange.startIdx),
              width: widthPct(selectRange.startIdx, selectRange.endIdx),
              height: STRIP_LANE_H - 2,
            }}
          />
        )}
        ...
        {bars.map((bar) => {
          const live =
            drag?.lodgingId === bar.lodging.id
              ? drag.preview
              : { startIdx: bar.startIdx, endIdx: bar.endIdx };
          const overlapping = bars.some(
            (o) =>
              o !== bar &&
              !(live.endIdx < o.startIdx || live.startIdx > o.endIdx),
          );
          const isHovered =
            hovered === bar.lodging.id && drag?.lodgingId !== bar.lodging.id;
          return (
            <div
              key={bar.lodging.id}
              className="absolute"
              style={{
                top: bar.lane * STRIP_LANE_H,
                left: pct(live.startIdx),
                width: widthPct(live.startIdx, live.endIdx),
              }}
            >
              {isHovered && (
                <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 z-20 whitespace-nowrap rounded-md bg-ink text-canvas text-xs px-2 py-1 shadow-lg pointer-events-none">
                  ...
                  <span className="opacity-70">
                    {" "}
                    · {fmtNight(nights[live.startIdx])} →{" "}
                    {fmtNight(addDaysIso(nights[live.endIdx], 1))}
                  </span>
                </div>
              )}
              <div
                ...
                onMouseLeave={() =>
                  setHovered((h) => (h === bar.lodging.id ? null : h))
                }
                ...
                style={{
                  height: STRIP_LANE_H - 2,
                  backgroundImage: daySeams(live.endIdx - live.startIdx + 1),
                }}
              >
                <div
                  onPointerDown={(e) => startBarDrag(bar, "resize-start", e)}
                  className="w-2 h-full cursor-ew-resize touch-none shrink-0"
                />
                <div
                  onPointerDown={(e) => startBarDrag(bar, "move", e)}
                  className="flex-1 h-full cursor-grab touch-none relative"
                >
                  {overlapping && (
                    <AlertTriangle className="w-3 h-3 text-white absolute inset-0 m-auto" />
                  )}
                </div>
                <div
                  onPointerDown={(e) => startBarDrag(bar, "resize-end", e)}
                  className="w-2 h-full cursor-ew-resize touch-none shrink-0"
                />
              </div>
            </div>
              ...
              <li key={l.id}>
                  ...
                  <span className="shrink-0 w-4 sm:w-24 inline-flex items-center">
                    <span
                      className={`inline-flex items-center gap-1.5 max-w-full ${
                        metro
                          ? "sm:pl-1.5 sm:pr-2 sm:py-0.5 sm:rounded-md sm:bg-surface sm:border sm:border-line-strong"
                          : ""
                      }`}
                    >
                      <span
                        className={`w-2 h-2 rounded-sm shrink-0 ${PALETTE[i % PALETTE.length]}`}
                        aria-hidden
                      />
                      {metro && (
                ...
                {!covers && (
                  <p className="pl-6 sm:pl-[6.5rem] text-[11px] text-danger-600 dark:text-danger-400">
                    Outside the trip&rsquo;s nights — no block on the strip
                    above.
                  </p>
                )}
              </li>
      ...
      {pendingAssign && (
        <div className="card p-3 space-y-2">
          <p className="text-xs text-faint">
            Assign lodging for {fmtNight(nights[pendingAssign.startIdx])} →{" "}
            {fmtNight(addDaysIso(nights[pendingAssign.endIdx], 1))}
          </p>
          <AssignExisting
            activities={activities}
            onCancel={() => setPendingAssign(null)}
            onAssign={async (locationId) => {
              await saveLodgingDates(locationId, {
                checkInDate: nights[pendingAssign.startIdx],
                checkOutDate: addDaysIso(nights[pendingAssign.endIdx], 1),
              });
          ...
          return (
            <div className="card p-3 space-y-2">
              ...
              <div className="flex items-center gap-1.5 text-xs">
                ...
                <input
                  type="date"
                  value={l.checkOutDate}
                  min={l.checkInDate}
                  max={addDaysIso(trip.endDate, 1)}
                  onChange={(e) =>
                    saveLodgingDates(l.id, {
      ...
      {uncoveredMetros.length > 0 && (
        <button
          ...
          className="text-xs text-faint hover:text-brand-600 dark:hover:text-brand-400 underline underline-offset-2"
        >
          Assign lodging for {uncoveredMetros.length}{" "}
          {uncoveredMetros.length === 1 ? "city" : "cities"} →
        </button>
      )}
      {wizardOpen && (
        <NightStripWizard
          metros={uncoveredMetros}
