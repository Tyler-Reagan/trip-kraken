...
function mockFetch(handler: (url: string) => unknown) {
  global.fetch = (async (url: string) => ({ ok: true, status: 200, json: async () => handler(url) }) as Response) as typeof fetch;
}

function mockFetchError(status: number, text: string) {
  global.fetch = (async () => ({ ok: false, status, text: async () => text }) as Response) as typeof fetch;
}
...
// ── costMatrix: URL shape — profile, coordinate order, annotations, no fallback_speed ──
{
  ...
  const matrix = await osrmProvider.costMatrix([P(35.68, 139.76), P(35.71, 139.79)], ["walking"]);

  assert.ok(capturedUrl.startsWith("http://localhost:5002/table/v1/foot/"), "walking resolves the foot profile and URL");
  assert.ok(capturedUrl.includes("139.76,35.68;139.79,35.71"), "coordinates are lng,lat, not lat,lng");
  assert.ok(capturedUrl.includes("annotations=duration,distance"));
  assert.ok(!capturedUrl.includes("fallback_speed"), "fallback_speed is never sent (ADR-0024's 2026-08-11 amendment)");
...
// ── costMatrix: driving resolves the car profile/URL ──
{
  ...
  await osrmProvider.costMatrix([P(35.68, 139.76), P(35.71, 139.79)], ["driving"]);
  assert.ok(capturedUrl.startsWith("http://localhost:5010/table/v1/car/"), "driving resolves the car profile and URL");
}
...
// ── costMatrix: a null duration cell declines; other cells in the same response still answer ──
{
  ...
  const matrix = await osrmProvider.costMatrix([P(0, 0), P(1, 1)], ["walking"]);
  assert.equal(matrix[0][1], null, "OSRM's own null duration declines that cell");
  assert.ok(matrix[1][0], "an unrelated cell in the same response still answers");
}
...
//    the out-of-Extract case, proven by shape, not by trusting a single-cell check ──
{
  ...
  const matrix = await osrmProvider.costMatrix([P(0, 0), P(1, 1), P(34.4, 132.45)], ["driving"]);
...
}
{
  ...
  await assert.rejects(() => osrmProvider.costMatrix([P(0, 0), P(1, 1)], ["driving"]), /InvalidQuery/);
}
...
//    bike collapse together; a ferry run becomes OtherPath; geometry is concatenated ──
{
              ...
              steps: [
                {
                  ...
                  mode: "walking",
                  name: "Main St",
                  geometry: { type: "LineString", coordinates: [[139.76, 35.68], [139.765, 35.685]] },
                },
                {
                  distance: 500,
                  duration: 300,
                  mode: "pushing bike",
                  name: "",
                  geometry: { type: "LineString", coordinates: [[139.765, 35.685], [139.77, 35.69]] },
                },
                {
                  distance: 2000,
                  duration: 900,
                  ...
                  geometry: { type: "LineString", coordinates: [[139.77, 35.69], [139.8, 35.7]] },
                },
                {
                  distance: 1500,
                  duration: 1800,
                  mode: "driving",
                  name: "Highway 1",
                  geometry: { type: "LineString", coordinates: [[139.8, 35.7], [139.79, 35.71]] },
                },
              ],
            },
          ],
  ...
  assert.ok(capturedUrl.includes("139.76,35.68;139.79,35.71"), "route URL is also lng,lat");
  assert.ok(paths);
  assert.equal(paths!.length, 3, "four steps collapse to three runs — walking absorbs pushing bike");
...
// ── describeJourney: a far-snapped waypoint declines the whole journey ──
{
  ...
  const declined = await osrmProvider.describeJourney({ lat: 35.68, lng: 139.76 }, { lat: 34.4, lng: 132.45 }, ["walking"]);
  assert.equal(declined, null, "an out-of-Extract endpoint declines the journey, not just a cell");
}
