...
function buildFixture(): TransitGraph {
  ...
  graph.rideEdges.push({
    fromStopId: "marunouchi-tokyo",
    toStopId: "marunouchi-otemachi",
    distanceMeters: 900,
  });
  ...
  graph.transferEdges.push({
    fromStopId: "yamanote-tokyo",
    toStopId: "marunouchi-tokyo",
    clusterId: "cluster-tokyo",
  });
  graph.transferEdges.push({
    fromStopId: "marunouchi-tokyo",
    toStopId: "yamanote-tokyo",
    clusterId: "cluster-tokyo",
  });
...
function normalize(graph: TransitGraph) {
  return {
    stopNodes: [...graph.stopNodes.entries()].sort(([a], [b]) =>
      a.localeCompare(b),
    ),
    clusters: [...graph.clusters.entries()]
      .map(
        ([id, c]) =>
          [id, { ...c, stopNodeIds: [...c.stopNodeIds].sort() }] as const,
      )
      .sort(([a], [b]) => a.localeCompare(b)),
    rideEdges: [...graph.rideEdges].sort((a, b) =>
      a.fromStopId.localeCompare(b.fromStopId),
    ),
    transferEdges: [...graph.transferEdges].sort((a, b) =>
      a.fromStopId.localeCompare(b.fromStopId),
    ),
  };
}
...
assert.throws(
  ...
  "missing graph file must throw loudly",
);
...
assert.deepEqual(
  normalize(reloaded),
  normalize(original),
  "reloaded graph matches the original structurally",
);
...
assert.equal(
  spatialIndex.nearby(35.6812, 139.7671, 200).length,
  2,
  "no other stop is within 200m of Tokyo Station",
);
assert.equal(
  spatialIndex.nearby(0, 0, 200).length,
  0,
  "an empty-ocean point far from any stop finds nothing",
);
...
assert.deepEqual(
  normalize(resaved),
  normalize(original),
  "re-saving over an existing file produces the same graph",
);
...
try {
  ...
  assert.equal(
    first,
    second,
    "getTransitGraph returns the same cached instance on repeat calls",
  );
} finally {
...
const withGeometry = reloaded.rideEdges.find(
  (e) => e.fromStopId === "yamanote-tokyo",
)!;
const withoutGeometry = reloaded.rideEdges.find(
  (e) => e.fromStopId === "marunouchi-tokyo",
)!;

assert.ok(
  withGeometry.geometry,
  "a ride edge's traced shape survives the round trip",
);
assert.equal(
  withGeometry.geometry!.coordinates.length,
  4,
  "every vertex comes back",
);
assert.equal(
  withGeometry.tracedLengthMeters,
  1289.4,
  "traced track length round-trips exactly — it is a plain REAL",
);
...
for (const [i, [lng, lat]] of withGeometry.geometry!.coordinates.entries()) {
  ...
  assert.ok(
    Math.abs(lng - wantLng) <= 1e-5 && Math.abs(lat - wantLat) <= 1e-5,
    `vertex ${i} within 5dp`,
  );
}
...
assert.equal(
  withoutGeometry.geometry,
  undefined,
  "an untraced ride edge comes back with no geometry at all",
);
assert.equal(
  withoutGeometry.tracedLengthMeters,
  undefined,
  "and no traced length",
);
...
const meta = metaDb
  .prepare("SELECT snapshotDate, region, ingestedAt FROM Meta")
  .all() as {
  snapshotDate: string;
  region: string;
  ingestedAt: string;
...
assert.equal(
  meta[0].snapshotDate,
  "260101",
  "the graph file records which Extract built it",
);
assert.equal(meta[0].region, "asia/japan");
assert.ok(
  !Number.isNaN(Date.parse(meta[0].ingestedAt)),
  "ingestedAt is a parseable timestamp",
);
