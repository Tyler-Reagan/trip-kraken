...
async function main() {
  ...
  async function describeJourney(
    ...args: Parameters<typeof googleRoutesProvider.describeJourney>
  ) {
  ...
  mockFetch((_url, init) => {
    ...
    assert.equal(
      body.departureTime,
      undefined,
      "walking never sends departureTime",
    );
    return [
      {
        originIndex: 0,
  ...
  await googleRoutesProvider.costMatrix([P(0, 0)], ["walking"], {
  ...
  assert.equal(
    calls,
    4,
    "11 points at 10x10 cap tiles into 2x2 = 4 chunk requests",
  );
  assert.equal(tiled.length, 11, "full 11x11 matrix stitched");
  assert.equal(
    tiled[10][10]!.distanceMeters,
    1,
    "far corner cell populated by the last chunk",
  );
  assert.equal(
    tiled[0][10]!.distanceMeters,
    1,
    "cross-chunk cell populated correctly",
  );
  ...
  const declinedMatrix = await googleRoutesProvider.costMatrix(
    [P(0, 0)],
    ["walking"],
  );
  assert.equal(
    declinedMatrix[0][0],
    null,
    "no-route condition declines the cell instead of throwing",
  );
  ...
  const journey = await describeJourney(P(35.68, 139.76), P(35.71, 139.79), [
    "rail",
  ]);
  assert.equal(
    journey.length,
    1,
    "single-element Path[] — decomposition unimplemented",
  );
  assert.equal(
    journey[0].kind,
    undefined,
    "no vehicle.type requested yet, so no honest kind to report",
  );
  assert.equal(
    journey[0].travelCost.durationSeconds,
    1800,
    "Path duration mapped",
  );
  assert.equal(
    journey[0].travelCost.basisOfCost,
    "routingService",
    "still a real routed cost, despite the unknown kind",
  );

  const busJourney = await describeJourney(P(35.68, 139.76), P(35.71, 139.79), [
    "bus",
  ]);
  assert.equal(
    busJourney[0].kind,
    undefined,
    "bus also folds onto Google's TRANSIT with no derivable kind yet",
  );
  ...
  assert.equal(
    walkJourney[0].kind,
    "walking",
    "walking resolves to WalkingPath",
  );

  // ── describeJourney: rail precedes walking in the resolution precedence ──
  mockFetch(() => ({ routes: [{ distanceMeters: 8000, duration: "1800s" }] }));
  const mixedJourney = await describeJourney(
    P(35.68, 139.76),
    P(35.71, 139.79),
    ["walking", "rail"],
  );
  assert.equal(
    mixedJourney[0].kind,
    undefined,
    "rail wins the precedence over walking, so still an unknown-kind transit result",
  );
  ...
  mockFetch((_url, init) => {
    ...
    assert.equal(
      body.departureTime,
      undefined,
      "driving never sends departureTime",
    );
  ...
  await assert.rejects(
    () => googleRoutesProvider.costMatrix([P(0, 0), P(1, 1)], ["rail"]),
    /HTTP 403/,
    "non-ok HTTP response throws",
  );
