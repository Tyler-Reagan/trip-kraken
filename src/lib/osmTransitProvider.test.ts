...
import {
  createOsmTransitProvider,
  snapStations,
  LINE_TYPE_SPEEDS_KMH,
  PREMIUM_BOARDING_MINUTES,
} from "./osmTransitProvider";
...
async function main() {
  function buildFixture(): TransitGraph {
    ...
    graph.rideEdges.push({
      fromStopId: "loop-kanda",
      toStopId: "loop-akihabara",
      distanceMeters: 1300,
      geometry: {
        type: "LineString",
        coordinates: [
    ...
    graph.stopNodes.set("spur-tokyo", {
      id: "spur-tokyo",
      osmNodeId: "spur-tokyo",
      lineId: "spur",
      lineName: "Spur Subway",
      lineType: "subway",
      stationName: "Tokyo",
      lat: 35.6812,
      lng: 139.7671,
    ...
    graph.rideEdges.push({
      fromStopId: "spur-tokyo",
      toStopId: "spur-otemachi",
      distanceMeters: 900,
    });
    ...
    graph.clusters.set("cluster-tokyo", {
      id: "cluster-tokyo",
      name: "Tokyo",
      stopNodeIds: ["loop-tokyo", "spur-tokyo"],
    });
    graph.transferEdges.push({
      fromStopId: "loop-tokyo",
      toStopId: "spur-tokyo",
      clusterId: "cluster-tokyo",
    });
    graph.transferEdges.push({
      fromStopId: "spur-tokyo",
      toStopId: "loop-tokyo",
    ...
    graph.stopNodes.set("shinkansen-tokyo", {
      id: "shinkansen-tokyo",
      osmNodeId: "shinkansen-tokyo",
      lineId: "shinkansen",
      lineName: "Tokaido Shinkansen",
      lineType: "shinkansen",
      stationName: "Tokyo (Shinkansen)",
      lat: 35.6812,
      lng: 139.7671,
      sequence: 0,
    });
    graph.stopNodes.set("shinkansen-nagoya", {
      id: "shinkansen-nagoya",
      osmNodeId: "shinkansen-nagoya",
      lineId: "shinkansen",
      lineName: "Tokaido Shinkansen",
      lineType: "shinkansen",
      stationName: "Nagoya",
    ...
    graph.rideEdges.push({
      fromStopId: "shinkansen-tokyo",
      toStopId: "shinkansen-nagoya",
      distanceMeters: 260_000,
    });
    ...
    graph.stopNodes.set("compare-subway-a", {
      id: "compare-subway-a",
      osmNodeId: "compare-subway-a",
      lineId: "compare-subway",
      lineName: "Compare Subway",
      lineType: "subway",
      stationName: "Compare A",
      lat: 35.0,
      lng: 139.0,
      sequence: 0,
    });
    graph.stopNodes.set("compare-subway-b", {
      id: "compare-subway-b",
      osmNodeId: "compare-subway-b",
      lineId: "compare-subway",
      lineName: "Compare Subway",
      lineType: "subway",
      stationName: "Compare B",
      lat: 35.0,
      lng: 141.0,
      sequence: 1,
    });
    graph.rideEdges.push({
      fromStopId: "compare-subway-a",
      toStopId: "compare-subway-b",
      distanceMeters: 260_000,
    });
    ...
    graph.stopNodes.set("decoy-near-akihabara", {
      id: "decoy-near-akihabara",
      osmNodeId: "decoy-near-akihabara",
      lineId: "decoy",
      lineName: "Decoy Line",
      lineType: "commuter",
      stationName: "Decoy",
    ...
    graph.stopNodes.set("loop-outlying", {
      id: "loop-outlying",
      osmNodeId: "loop-outlying",
      lineId: "loop",
      lineName: "Loop Line",
      lineType: "commuter",
      stationName: "Outlying",
      lat: 35.8,
      lng: 139.7731,
      sequence: 3,
    });
    graph.rideEdges.push({
      fromStopId: "loop-akihabara",
      toStopId: "loop-outlying",
    ...
    graph.stopNodes.set("through-a-start", {
      id: "through-a-start",
      osmNodeId: "through-a-start",
      lineId: "through-a",
      lineName: "Through Line A",
      lineType: "commuter",
      stationName: "Through Start",
      lat: 34.0,
      lng: 135.0,
      sequence: 0,
    });
    graph.stopNodes.set("through-a-shared", {
      id: "through-a-shared",
      osmNodeId: "through-shared-point",
      lineId: "through-a",
      lineName: "Through Line A",
      lineType: "commuter",
      stationName: "Through Junction",
      lat: 34.01,
      lng: 135.01,
      sequence: 1,
    });
    graph.rideEdges.push({
      fromStopId: "through-a-start",
      toStopId: "through-a-shared",
      distanceMeters: 4000,
    });
    ...
    graph.stopNodes.set("through-b-end", {
      id: "through-b-end",
      osmNodeId: "through-b-end",
      lineId: "through-b",
      lineName: "Through Line B",
      lineType: "subway",
      stationName: "Through End",
      lat: 34.02,
      lng: 135.02,
      sequence: 1,
    });
    graph.rideEdges.push({
      fromStopId: "through-b-shared",
      toStopId: "through-b-end",
      distanceMeters: 3000,
    });
  ...
  assert.equal(
    (multiLine[1] as { lineName: string }).lineName,
    "Loop Line",
    "each rail Path names one line, not a joined string",
  );
  ...
  assert.equal(
    multiLine[2].from.stationName,
    "Tokyo",
    "the transfer's endpoints carry the station cluster's name",
  );
  assert.equal(multiLine[2].to.stationName, "Tokyo");
  assert.equal(
    multiLine[2].travelCost.distanceMeters,
    0,
    "a transfer has no modeled distance, only TRANSFER_MINUTES (§4)",
  );
  assert.equal(
    multiLine[2].travelCost.basisOfCost,
    "straightLine",
    "and says so — nothing routed it",
  );

  // A rail Path's own ends are nameable too, which is what #139's sidebar reads.
  assert.equal(
    multiLine[1].from.stationName,
    "Akihabara",
    "a rail Path knows where it boarded",
  );
  assert.equal(multiLine[1].to.stationName, "Tokyo", "and where it alighted");
  assert.equal(
    multiLine[1].travelCost.basisOfCost,
    "railNetwork",
    "a ride is graph traversal, not an estimate",
  );
  ...
  assert.equal(
    multiLine[1].from.locationId,
    undefined,
    "an interchange endpoint carries no locationId",
  );
  ...
  const matrixCell = (
    await provider.costMatrix([nearAkihabara, nearOtemachi], ["rail"])
  )[0][1]!;
  assert.ok(
  ...
  assert.equal(
    summed.basisOfCost,
    "railNetwork",
    "the summed Journey reports the basis of its dominant Path, not its weakest",
  );
  ...
  const throughJourney = await describe(nearThroughStart, nearThroughEnd, [
    "rail",
  ]);
  assert.deepEqual(
  ...
  assert.equal(
    (throughJourney[1] as { lineName: string }).lineName,
    "Through Line A",
  );
  assert.equal(
    (throughJourney[2] as { lineName: string }).lineName,
    "Through Line B",
  );
  const throughRailMinutes =
    throughJourney[1].travelCost.durationSeconds / 60 +
    throughJourney[2].travelCost.durationSeconds / 60;
  const expectedThroughRailMinutes =
    (4000 / 1000 / LINE_TYPE_SPEEDS_KMH.commuter) * 60 +
    (3000 / 1000 / LINE_TYPE_SPEEDS_KMH.subway) * 60;
  assert.ok(
  ...
  assert.equal(
    (singleRideRail as { lineName: string }).lineName,
    "Loop Line",
    "single-line journey reports just that line",
  );
  assert.equal(
  ...
  const walkOnly = await describe(P(35.6983, 139.7733), P(35.6985, 139.773), [
    "rail",
  ]);
  assert.deepEqual(
    walkOnly.map((p) => p.kind),
    ["walking"],
    "a rideless Journey is one walking Path",
  );
  assert.equal(walkOnly[0].from.locationId, undefined);
  assert.equal(
    walkOnly[0].geometry,
    undefined,
    "with no shape — nothing routed it",
  );
  ...
  const shinkansenJourney = await describe(nearTokyoForShinkansen, nearNagoya, [
    "rail",
  ]);
  const equalDistanceSubwayJourney = await describe(
    nearCompareA,
    nearCompareB,
    ["rail"],
  );
  assert.ok(
    journeyCost(shinkansenJourney)!.durationSeconds <
      journeyCost(equalDistanceSubwayJourney)!.durationSeconds,
  ...
  assert.ok(
    Math.abs(
      shinkansenRail.travelCost.durationSeconds / 60 -
  ...
  assert.equal(
    PREMIUM_BOARDING_MINUTES.commuter,
    0,
    "an ordinary commuter line carries no boarding charge",
  );
  assert.ok(
    Math.abs(
      commuterRail.travelCost.durationSeconds / 60 -
        (2500 / 1000 / LINE_TYPE_SPEEDS_KMH.commuter) * 60,
    ) < 1e-6,
  ...
  const shinkansenMatrix = await provider.costMatrix(
    [nearTokyoForShinkansen, nearNagoya],
    ["rail"],
  );
  assert.ok(
    Math.abs(
      shinkansenMatrix[0][1]!.durationSeconds -
        journeyCost(shinkansenJourney)!.durationSeconds,
    ) < 1e-6,
  ...
  assert.ok(
    nearestStops.some((s) => s.id === "decoy-near-akihabara"),
    "the farther decoy is in range too",
  );
  assert.equal(
    nearestStops[0]?.id,
    "loop-akihabara",
    "the nearest stop in range is Akihabara, not the farther decoy",
  );
  ...
  assert.equal(
    outlyingSnaps[0]?.id,
    "loop-outlying",
    "the one walkable station beyond the dense radius is found",
  );

  const outlyingJourney = await describe(aboveOutlyingStation, nearAkihabara, [
    "rail",
  ]);
  assert.ok(
    outlyingJourney.some((p) => p.kind === "rail"),
    "an isolated-access journey still rides rail",
  );
  ...
  assert.equal(
    outlyingJourney[0].kind,
    "walking",
    "and it leads the chain as its own Path",
  );
  assert.ok(
  ...
  const noStationJourney = await provider.describeJourney(
    isolated,
    nearAkihabara,
    ["rail"],
  );
  assert.equal(
    noStationJourney,
    null,
    "no station within either radius declines the whole journey",
  );

  // ── costMatrix backs the same provider surface in bulk, consistent with describeJourney ──
  const matrix = await provider.costMatrix(
    [nearAkihabara, nearOtemachi, isolated],
    ["rail"],
  );
  ...
  assert.equal(
    matrix[0][2],
    null,
    "an isolated point declines rather than getting a fabricated walking estimate",
  );
  assert.equal(
    matrix[0][0],
    null,
    "a point costed against itself also declines — the terminal entry gives the same zero for free",
  );
  ...
  const storedKandaHop = graph.rideEdges.find(
    (e) => e.fromStopId === "loop-kanda",
  )!.geometry!;
  ...
  assert.equal(
    JSON.stringify(storedKandaHop.coordinates),
    storedBefore,
    "and the stored shape is untouched",
  );
  ...
  assert.equal(
    multiLine[1].geometry?.length,
    2,
    "both traced spans belong to the line that was actually ridden over them",
  );
  assert.equal(
    multiLine[3].geometry,
    undefined,
    "the untraced ride carries no shape at all, not an empty list",
  );
  ...
  assert.ok(
    spurOnly.every((p) => p.geometry === undefined),
    "an entirely untraced Journey carries no geometry",
  );
  ...
  const geometryMatrix = await provider.costMatrix(
    [nearAkihabara, nearOtemachi],
    ["rail"],
  );
  assert.equal(
  ...
  function buildJrPassFixture(): TransitGraph {
    ...
    jrPassGraph.stopNodes.set("privateA", {
      ...
      osmNodeId: "privateA",
      lineId: "private-line",
      lineName: "Private Line",
      lineType: "commuter",
      operator: "Meitetsu",
      stationName: "Private A",
      lat: 34.8,
      lng: 135.5,
      sequence: 0,
    });
  ...
  const jrPassProvider = createOsmTransitProvider(
    jrPassGraph,
    buildSpatialIndex(jrPassGraph),
  );

  const near = (stop: { lat: number; lng: number }) =>
    P(stop.lat + 0.00005, stop.lng + 0.00005);
  ...
  const jrUnderPass = await jrPassProvider.describeJourney(jrA, jrB, ["rail"], {
    hasJrPass: true,
  });
  assert.ok(
    jrUnderPass,
    "a confirmed JR-operator line stays routable under a JR Pass",
  );
  ...
  const privateUnderPass = await jrPassProvider.describeJourney(
    privateA,
    privateB,
    ["rail"],
    { hasJrPass: true },
  ...
  assert.equal(
    privateUnderPass,
    null,
    "a confirmed non-JR line is excluded under a Pass, declining rather than throwing",
  );
  const privateMatrixUnderPass = await jrPassProvider.costMatrix(
    [privateA, privateB],
    ["rail"],
    { hasJrPass: true },
  );
  assert.equal(
    privateMatrixUnderPass[0][1],
    null,
    "costMatrix declines the same excluded pair, not just describeJourney",
  );
  ...
  const unknownUnderPass = await jrPassProvider.describeJourney(
    unknownA,
    unknownB,
    ["rail"],
    { hasJrPass: true },
  ...
  assert.ok(
    unknownUnderPass,
    "an unknown-operator line stays routable under a Pass, not excluded on absence of data",
  );
  ...
  for (const opts of [
    ...
    const nozomiJourney = await jrPassProvider.describeJourney(
      nozomiA,
      nozomiB,
      ["rail"],
      opts,
    ...
    assert.ok(
      nozomiJourney,
      "Nozomi always routes — the named-train exclusion is never a hard exclude",
    );
    const rail = nozomiJourney!.find((p) => p.kind === "rail") as {
      jrPassSupplementRequired?: boolean;
      operator?: { name: string };
    };
    assert.equal(
      rail.jrPassSupplementRequired,
      true,
      "a Nozomi leg is flagged regardless of the query's own hasJrPass",
    );
    assert.equal(
      rail.operator?.name,
      "JR Central",
      "the leg's operator is populated from the captured StopNode field (issue #210)",
    );
  }
  ...
  const plainJrJourney = await jrPassProvider.describeJourney(jrA, jrB, [
    "rail",
  ]);
  assert.ok(plainJrJourney, "the plain JR line still routes");
  const plainJrRail = plainJrJourney!.find((p) => p.kind === "rail") as {
    jrPassSupplementRequired?: boolean;
  };
  assert.equal(
    plainJrRail.jrPassSupplementRequired,
    undefined,
