...
async function main() {
  ...
  assert.equal(
    ...
    "New searchText endpoint",
  );
  assert.equal(lastRequest!.init.method, "POST", "POST request");
  assert.equal(
    requestHeader("X-Goog-Api-Key"),
    "test-key",
    "key in header, not URL",
  );
  assert.ok(
    requestHeader("X-Goog-FieldMask")?.includes("places.id"),
    "field mask requests place id",
  );
  assert.deepEqual(
    requestBody(),
    { textQuery: "Senso-ji Tokyo", pageSize: 20 },
    "textQuery + pageSize body",
  );
  ...
  assert.equal(
    r.detourMeters,
    null,
    "unanchored → no detour (routingSummaries never requested)",
  );

  // ── searchText: zero results is an empty object, not a status field ──
  mockFetch({});
  assert.deepEqual(
    await searchText("asdfqwerty nowhere"),
    [],
  ...
  mockFetch(
    { error: { code: 403, message: "bad key", status: "PERMISSION_DENIED" } },
    false,
    403,
  );
  ...
  assert.equal(
    ...
    "New searchNearby endpoint",
  );
  assert.deepEqual(
    requestBody(),
    {
      locationRestriction: {
        circle: { center: { latitude: 35.71, longitude: 139.79 }, radius: 800 },
      },
      maxResultCount: 5,
    },
  ...
  mockFetch({ places: [newPlace] });
  await searchNearby(35.71, 139.79, {
    radius: 800,
    keyword: "kissaten",
    openNow: true,
  });
    ...
    places: [
      {
        ...newPlace,
        id: "inside",
        location: { latitude: 35.712, longitude: 139.797 },
      },
      {
        ...newPlace,
        id: "far-away",
        location: { latitude: 35.0, longitude: 135.75 },
      }, // Kyoto, ~370km
    ],
  });
  results = await searchNearby(35.71, 139.79, {
    radius: 1000,
    keyword: "ramen",
  });
  assert.deepEqual(
    results.map((p) => p.placeId),
    ["inside"],
    "beyond-radius text results filtered",
  );
  ...
  assert.ok(
    requestHeader("X-Goog-FieldMask")?.includes(
  ...
  mockFetch({ places: [newPlace] });
  results = await searchAlongRoute(
    "bakery",
    "abc123",
    { lat: 35.71, lng: 139.79 },
    { limit: 5, openNow: true },
  );
  assert.equal(
    lastRequest!.url,
    "https://places.googleapis.com/v1/places:searchText",
    "along-route uses text search",
  );
  assert.deepEqual(
    requestBody(),
    {
  ...
  assert.equal(
    results[0].detourMeters,
    null,
    "no routingSummaries in response → null detourMeters",
  );
  ...
  mockFetch({
    ...
    routingSummaries: [
      { legs: [{ distanceMeters: 400 }] },
      { legs: [{ distanceMeters: 3200 }] },
    ],
  });
  results = await searchAlongRoute("bakery", "abc123", {
    lat: 35.71,
    lng: 139.79,
  });
  assert.deepEqual(
  ...
  mockFetch({
    places: [
      { id: "place-2", location: { latitude: 34.66, longitude: 135.5 } },
    ],
  });
  const found = await findPlaceFromText("Ichiran Namba", 34.67, 135.5, 5000);
  assert.deepEqual(
    found,
    { placeId: "place-2", lat: 34.66, lng: 135.5 },
    "id + coords resolved",
  );
  assert.deepEqual(
    requestBody(),
    {
      ...
      pageSize: 1,
      locationBias: {
        circle: { center: { latitude: 34.67, longitude: 135.5 }, radius: 5000 },
      },
    },
  ...
  mockFetch({
    places: [
      {
        id: "place-far",
        location: { latitude: 50.1162, longitude: -122.9535 },
      },
    ],
  });
  assert.equal(
  ...
  mockFetch({
    places: [
      {
        id: "place-near",
        location: { latitude: 35.7105, longitude: 139.7905 },
      },
    ],
  });
  assert.deepEqual(
  ...
  mockFetch({});
  assert.equal(
    await findPlaceFromText("nowhere", null, null),
    null,
    "zero results → null",
  );
  global.fetch = (async () => {
  ...
  assert.equal(
    await findPlaceFromText("x", null, null),
    null,
    "fetch throw → null",
  );
  ...
  assert.equal(
    lastRequest!.url,
    "https://places.googleapis.com/v1/places/place-3",
    "place resource GET",
  );
  assert.equal(lastRequest!.init.method ?? "GET", "GET", "details is a GET");
  assert.ok(
    requestHeader("X-Goog-FieldMask")?.includes("regularOpeningHours"),
    "mask requests hours",
  );
  ...
  assert.deepEqual(
    details?.categories,
    ["restaurant"],
    "generic types stripped",
  );
  assert.equal(
    details?.openTime,
    "09:30",
    "Monday open, {hour,minute} → HH:MM",
  );
