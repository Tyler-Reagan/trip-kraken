...

{
  ...
  assert.deepEqual(
    day1,
    [],
    "source day is empty after its only placement moves away",
  );
  assert.deepEqual(
    day2.map((x) => x.id),
    ["a", "b"],
    "moved placement lands ahead of the existing one",
  );
}
...

{
  ...
  const next = insertPlacement(placements, "trip-1", {
    id: "c",
    locationId: "loc-c",
    date: "2026-01-01",
  });
  ...
  assert.deepEqual(
    day1.map((x) => x.id),
    ["a", "b", "c"],
    "omitted order appends to the end of the date",
  );
}
...

{
  ...
  assert.deepEqual(
    day1.map((x) => x.id),
    ["c", "a", "b"],
    "explicit order inserts and shifts siblings down",
  );
}
...
{
  const next = insertPlacement([], "trip-1", {
    id: "a",
    locationId: "loc-a",
    date: "2026-01-01",
  });
  assert.deepEqual(
    next,
    [p("a", "2026-01-01", 0, "loc-a")],
    "first placement on a date gets order 0",
  );
}
