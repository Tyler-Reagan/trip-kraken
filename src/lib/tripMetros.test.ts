...
assert.equal(
  metroLabel({ activities: [at("Sensō-ji", "2-chōme-3-1 Asakusa, Taito City, Tokyo 111-0032, Japan")] }),
  "Tokyo",
);

// ── localityOf: the unit *below* whatever metroLabel said ──
const locality = (address: string) => localityOf(address, metroLabel({ activities: [at("x", address)] }));

assert.equal(locality("1-chōme-12-10 Kitahorie, Nishi Ward, Osaka, 550-0014, Japan"), "Nishi Ward");
assert.equal(locality("2-chōme-5-5 Nakatsu, Kita Ward, Osaka, 531-0071, Japan"), "Kita Ward");
assert.equal(locality("Dotonbori, Chuo Ward, Osaka, 542-0071, Japan"), "Chuo Ward");
assert.equal(locality("1-chōme-1-10 Kaigandōri, Minato Ward, Osaka, 552-0022, Japan"), "Minato Ward");
assert.equal(locality("2-chōme-3-1 Asakusa, Taito City, Tokyo 111-0032, Japan"), "Taito City");
...
assert.equal(
  locality("2 Chome Kita 1 Jonishi, Chuo Ward, Sapporo, Hokkaido 060-0001, Japan"),
  "Sapporo",
...
assert.equal(localityOf("Osaka, Japan", "Osaka"), null, "nothing survives but the metro itself");
assert.equal(
