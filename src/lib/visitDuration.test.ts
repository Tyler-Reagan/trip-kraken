...
assert.equal(
  resolveVisitDuration(null),
  DEFAULT_VISIT_MINUTES,
  "null resolves to the default",
);
assert.equal(
  resolveVisitDuration(undefined),
  DEFAULT_VISIT_MINUTES,
  "undefined resolves to the default",
);
...
// ── clamp ──
assert.equal(
  clampVisitDuration(0),
  VISIT_DURATION_STEP_MINUTES,
  "clamps up to the floor",
);
assert.equal(
  clampVisitDuration(-99),
  VISIT_DURATION_STEP_MINUTES,
  "negatives clamp to the floor",
);
assert.equal(
  clampVisitDuration(9999),
  VISIT_DURATION_MAX_MINUTES,
...
assert.equal(
  nextVisitDuration(45, 1),
  60,
  "below 1h steps by 15 up to the boundary",
);
...
for (const start of VISIT_DURATION_LADDER) {
  ...
  assert.equal(
    nextVisitDuration(up, -1),
    start,
    `stepping up from ${start} then down returns to ${start}`,
  );
  const down = nextVisitDuration(start, -1);
  if (down === start) continue; // at the floor
  assert.equal(
    nextVisitDuration(down, 1),
    start,
    `stepping down from ${start} then up returns to ${start}`,
  );
}
...
assert.equal(
  nextVisitDuration(105, 1),
  120,
  "105m steps up to the next ladder stop, not 105+30",
);
assert.equal(
  nextVisitDuration(105, -1),
  90,
  "105m steps down to the previous ladder stop",
);
assert.ok(
  VISIT_DURATION_LADDER.includes(nextVisitDuration(105, 1)),
  "stepping always lands on the ladder",
);
...
assert.equal(
  nextVisitDuration(900, -1),
  VISIT_DURATION_MAX_MINUTES,
  "above the ceiling, − lands on the ceiling",
);

assert.equal(
  nextVisitDuration(VISIT_DURATION_STEP_MINUTES, -1),
  VISIT_DURATION_STEP_MINUTES,
  "the floor holds",
);
assert.equal(
  nextVisitDuration(VISIT_DURATION_MAX_MINUTES, 1),
  VISIT_DURATION_MAX_MINUTES,
  "the ceiling holds",
);
...
// ── roller options ──
assert.equal(
  VISIT_DURATION_OPTIONS[0],
  VISIT_DURATION_STEP_MINUTES,
  "the list starts at the floor",
);
assert.equal(
  VISIT_DURATION_OPTIONS.at(-1),
  VISIT_DURATION_MAX_MINUTES,
  "the list ends at the ceiling",
);
assert.ok(
...
assert.ok(
  !VISIT_DURATION_OPTIONS.includes(0),
  "zero is not representable — an invalid duration has no row",
);
...
assert.equal(
  VISIT_DURATION_OPTIONS[nearestVisitDurationIndex(20)],
  15,
  "20m (off-grid, API-reachable) snaps to 15m",
);
assert.equal(
  VISIT_DURATION_OPTIONS[nearestVisitDurationIndex(38)],
  45,
  "38m snaps to the nearer 45m",
);
assert.equal(
  VISIT_DURATION_OPTIONS[nearestVisitDurationIndex(1440)],
  VISIT_DURATION_MAX_MINUTES,
  "a value above the ceiling (the API allows up to 1440) clamps to the last row rather than the first",
);
assert.equal(
  nearestVisitDurationIndex(0),
  0,
  "zero clamps to the first row, never a negative index",
);
