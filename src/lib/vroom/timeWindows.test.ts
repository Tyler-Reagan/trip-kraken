...
const loc = (fields: Partial<LocationInput>): LocationInput => ({
  id: "l1",
  lat: 35,
  lng: 139,
  kind: "activity",
...
const dayStart = (date: string): number =>
...
// ── Rule 5: no hours data at all → unconstrained (null) ──
{
  ...
  assert.equal(
    windows,
    null,
    "no hoursJson and no openTime/closeTime → null, not []",
  );
}
...
// ── No Trip dates to evaluate → unconstrained (null), not a false "closed every day" ──
{
  const windows = dayWindowsFor(
    loc({ openTime: "09:00", closeTime: "18:00" }),
    [],
  );
...
{
  const windows = dayWindowsFor(
    loc({ hoursJson: { "3": { open: "09:00", close: "17:00" } } }),
    [MON, TUE],
  ); // Wed only, not requested
  assert.deepEqual(
    windows,
    [],
    "hours exist but never fall on a requested date → [], not null",
  );
}
...
{
  const windows = dayWindowsFor(
    loc({ hoursJson: { "1": { open: "10:00", close: null } } }),
    [MON],
  );
  assert.deepEqual(windows, [
    [dayStart(MON) + 10 * 3600, dayStart(MON) + SEC_PER_DAY],
  ]);
}
...
// ── Rule 3: close <= open clamps to day end (extractWeeklyHours flattens an overnight period) ──
{
  ...
  const windows = dayWindowsFor(
    loc({ hoursJson: { "1": { open: "18:00", close: "02:00" } } }),
    [MON],
  );
  assert.deepEqual(windows, [
    [dayStart(MON) + 18 * 3600, dayStart(MON) + SEC_PER_DAY],
  ]);
}
...
{
  const windows = dayWindowsFor(
    loc({ openTime: "08:00", closeTime: "20:00" }),
    [MON, TUE, WED],
  );
  assert.equal(
    windows!.length,
    3,
    "openTime/closeTime applies to every date, not just one",
  );
  assert.deepEqual(windows![1], [
    dayStart(TUE) + 8 * 3600,
    dayStart(TUE) + 20 * 3600,
  ]);
}
...
// ── hoursJson takes priority over openTime/closeTime when both are present ──
{
  ...
  assert.deepEqual(
    windows,
    [[dayStart(MON) + 9 * 3600, dayStart(MON) + 17 * 3600]],
    "hoursJson wins over openTime/closeTime",
  );
}
