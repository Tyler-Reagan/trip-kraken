...
function lodgingIdOnNight(stays: StayPlan[], night: number): string | null {
  return (
    stays.find((s) => night >= s.startNight && night <= s.endNight)
      ?.lodgingId ?? null
  );
}
...
function buildSolverInputDays(
  ...
  return Array.from({ length: numDays }, (_, i) => {
    ...
    if (dayNumber === 1 && arriveAtSeconds != null)
      opensAt = Math.max(opensAt, arriveAtSeconds);
    if (dayNumber === numDays && departAtSeconds != null)
      closesAt = Math.min(closesAt, departAtSeconds);
    if (opensAt > closesAt) closesAt = opensAt;

    let reachableMetros: number[] = [];
    if (metroActive) {
      const wakeMetros =
        wokeLodgingId != null ? (lodgingMetros.get(wokeLodgingId) ?? []) : [];
      const sleepMetros =
        sleepLodgingId != null ? (lodgingMetros.get(sleepLodgingId) ?? []) : [];
...
export function buildVroomRequest(input: BuildVroomRequestInput): VroomRequest {
  const { placeable, matrixPoints, matrix, tripDates, metroOf, lodgingMetros } =
    input;

  const indexOf = new Map(matrixPoints.map((p, i) => [p.id, i]));
  const allMetroOrdinals = [
    ...new Set([...metroOf.values(), ...[...lodgingMetros.values()].flat()]),
  ];
  ...
  const jobs: VroomJob[] = placeable.map((a) =>
    jobForActivity(a, indexOf.get(a.id)!, tripDates, metroOf),
  );
  const vehicles: VroomVehicle[] = tripDaysArr.map(vehicleForDay);

  const durations = matrix.map((row) =>
    row.map((cell) => Math.round(cell.durationSeconds)),
  );
