...
const solution: VroomSolution = {
  ...
  routes: [
    {
      vehicle: 1,
      steps: [{ type: "start" }, { type: "job", id: 0 }, { type: "end" }],
    },
  ],
...
async function main() {
  ...
  {
    const out = await diagnoseUnplaced(
      request,
      solution,
      unplaced,
      matrixPoints,
    );
    assert.deepEqual(out[0].diagnosis, {
      cause: "day-too-short",
      dayNumber: 1,
      seconds: 1800,
    });
    ...
    assert.deepEqual(
      out[1],
      unplaced[1],
      "a pre-flight reason already knows why — never probed, never rewritten",
    );
  }
  ...
  const failures: Array<[string, typeof fetch]> = [
    ...
    [
      ...
      (async () => ({
        ...
        json: async () => ({
          code: 2,
          error: "VROOM compiled without libglpk installed.",
          routes: [],
        }),
      })) as unknown as typeof fetch,
    ],
  ...
  for (const [label, stub] of failures) {
    ...
    assert.deepEqual(
      out,
      unplaced,
      `${label}: the Unplaced list survives untouched`,
    );
    assert.equal(
      out.every((u) => u.diagnosis === undefined),
      true,
      `${label}: no fabricated diagnosis`,
    );
  }
  ...
  // ── an unconfigured machine skips the pass rather than throwing ──
  {
    ...
    assert.deepEqual(
      await diagnoseUnplaced(request, solution, unplaced, matrixPoints),
      unplaced,
    );
  ...
  // ── nothing the solver dropped → no call at all ──
  {
    ...
    assert.deepEqual(
      await diagnoseUnplaced(request, solution, preflightOnly, matrixPoints),
      preflightOnly,
    );
    assert.equal(
      called,
      false,
      "a Trip with nothing solver-dropped never pays for the second call",
    );
  }
