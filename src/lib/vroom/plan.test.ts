...
// ── probe assignment: one Day per probe, and the Day is chosen to be informative ──
{
  ...
  assert.deepEqual(
    rounds[0],
    [{ jobId: 2, vehicleId: 3 }],
    "probes the Day whose skills serve its metro",
  );
}

{
  ...
  assert.equal(
    new Set(vehicles).size,
    3,
    "no Day carries two probes in one round",
  );
}

{
  ...
  assert.deepEqual(
    rounds[0],
    [{ jobId: 0, vehicleId: 2 }],
    "prefers a serving Day with room over one at its cap",
  );
}

{
  // More dropped Activities than Days → a second round rather than doubling up on a Day.
  const twoDays: VroomRequest = {
    ...request,
    vehicles: request.vehicles.slice(0, 2),
  };
  const many = planProbeRounds(twoDays, solution, [0, 1, 2]);
  assert.equal(
    many.length,
    2,
    "three Activities over two Days take two rounds",
  );
  ...
  assert.deepEqual(
    probed,
    [0, 1, 2],
    "every dropped Activity is probed exactly once, across rounds",
  );
}

{
  assert.deepEqual(
    planProbeRounds(request, solution, []),
    [],
    "nothing dropped, nothing probed",
  );
  assert.deepEqual(
    planProbeRounds(request, solution, [99]),
    [],
    "an unknown job id is not probed",
  );
}
...
// ── request building: the probe rides last, only probed Days are sent, plan mode is on ──
{
  ...
  assert.equal(
    planRequest.options.c,
    true,
    "plan mode must be requested explicitly",
  );
  ...
  assert.equal(
    planRequest.vehicles[0].max_tasks,
    2,
    "the Day's real constraints ride along unchanged",
  );
...

{
  ...
  const planRequest = buildVroomPlanRequest(request, solution, [
    { jobId: 2, vehicleId: 3 },
  ]);
  assert.deepEqual(planRequest.vehicles[0].steps, [
    { type: "start" },
    { type: "job", id: 2 },
    { type: "end" },
  ]);
...
// ── golden plan-mode response → domain findings ──
{
    ...
    routes: [
      {
        ...
        steps: [
          ...
          { type: "job", id: 0, violations: [] },
          {
            type: "job",
            id: 2,
            violations: [{ duration: 1100, cause: "delay" }],
          },
          { type: "end", violations: [] },
        ],
      },
  ...
  const found = parseVroomViolations(planSolution, [
    { jobId: 2, vehicleId: 1 },
  ]);
  assert.deepEqual(found.get(2), {
    cause: "after-closing",
    dayNumber: 1,
    seconds: 1100,
  });
...

{
    ...
    routes: [
      {
        ...
        steps: [
          {
            type: "start",
            violations: [{ duration: 5948, cause: "lead_time" }],
          },
  ...
  assert.deepEqual(
    parseVroomViolations(startSpill, [{ jobId: 2, vehicleId: 2 }]).get(2),
    {
      cause: "day-too-short",
      dayNumber: 2,
      seconds: 5948,
...

{
    ...
    routes: [
      {
        ...
        steps: [
          {
            type: "start",
            violations: [{ duration: 1115, cause: "lead_time" }],
          },
          { type: "job", id: 2, violations: [] },
  ...
  assert.deepEqual(
    parseVroomViolations(bothEnds, [{ jobId: 2, vehicleId: 1 }]).get(2),
    {
      cause: "day-too-short",
      dayNumber: 1,
      seconds: 12892,
...

{
    ...
    routes: [
      {
        ...
        steps: [
          { type: "start" },
          { type: "job", id: 2, violations: [{ cause: "skills" }] },
          { type: "end" },
        ],
      },
    ],
  ...
  assert.deepEqual(
    parseVroomViolations(skills, [{ jobId: 2, vehicleId: 3 }]).get(2),
    {
      cause: "out-of-reach",
      dayNumber: 3,
    },
    ...
    routes: [
      {
        ...
        steps: [
          { type: "start" },
          { type: "job", id: 2, violations: [{ cause: "max_tasks" }] },
          { type: "end" },
        ],
      },
    ],
  ...
  assert.deepEqual(
    parseVroomViolations(full, [{ jobId: 2, vehicleId: 1 }]).get(2),
    { cause: "day-full", dayNumber: 1 },
  );
}

{
    ...
    routes: [
      {
        ...
        steps: [
          {
            type: "start",
            violations: [{ duration: 300, cause: "lead_time" }],
          },
          {
            type: "job",
            id: 2,
            violations: [{ duration: 600, cause: "delay" }],
          },
  ...
  assert.deepEqual(
    parseVroomViolations(both, [{ jobId: 2, vehicleId: 1 }]).get(2),
    {
      cause: "after-closing",
      dayNumber: 1,
      seconds: 600,
    ...
    routes: [
      {
        ...
        steps: [
          { type: "start" },
          {
            type: "job",
            id: 2,
            violations: [{ duration: 1200, cause: "lead_time" }],
          },
          { type: "end" },
        ],
      },
  ...
  assert.deepEqual(
    parseVroomViolations(early, [{ jobId: 2, vehicleId: 1 }]).get(2),
    {
...

{
  ...
  assert.equal(
    parseVroomViolations(clean, [{ jobId: 2, vehicleId: 1 }]).size,
    0,
  );
    ...
    routes: [
      {
        ...
        steps: [
          { type: "start" },
          { type: "job", id: 2, violations: [{ cause: "load" }] },
          { type: "end" },
        ],
      },
    ],
  ...
  assert.equal(
    parseVroomViolations(unmapped, [{ jobId: 2, vehicleId: 1 }]).size,
    0,
  );
}

{
    ...
    routes: [
      {
        ...
        steps: [
          { type: "start" },
          { type: "job", id: 9, violations: [{ cause: "skills" }] },
          { type: "end" },
        ],
      },
    ],
  ...
  assert.equal(
    parseVroomViolations(stray, [{ jobId: 2, vehicleId: 1 }]).size,
    0,
  );
}
...
{
  assert.match(
    diagnosisReason({ cause: "after-closing", dayNumber: 3, seconds: 2400 }),
    /40 min after it closes on day 3/,
  );
  assert.match(
    diagnosisReason({ cause: "before-opening", dayNumber: 3, seconds: 600 }),
    /10 min before it opens on day 3/,
  );
  assert.match(
    diagnosisReason({ cause: "day-too-short", dayNumber: 2, seconds: 1800 }),
    /Day 2 would need 30 more min/,
  );
  assert.match(
    diagnosisReason({ cause: "day-full", dayNumber: 1 }),
    /already full/,
  );
  assert.match(
    diagnosisReason({ cause: "out-of-reach", dayNumber: 4 }),
    /near enough/,
  );

  // Sub-minute lateness rounds up: "1 min late" is honest where "0 min late" would read as fine.
  assert.match(
    diagnosisReason({ cause: "after-closing", dayNumber: 1, seconds: 30 }),
    /1 min after it closes/,
  );
  // No magnitude → a sentence that still reads, with no invented number in it.
  assert.match(
    diagnosisReason({ cause: "after-closing", dayNumber: 1 }),
    /before it closes on day 1/,
  );
}
