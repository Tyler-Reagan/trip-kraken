...
async function main() {
  ...
  assert.deepEqual(
    ...
    [
      ...
      {
        id: "haversine",
        kinds: ["rail", "bus", "walking", "driving", "bicycle", "other"],
        terminal: true,
      },
    ],
  ...
  assert.equal(
    entry("osm-japan").isAvailable([PARIS]),
    false,
    "declines outside Japan regardless of graph presence",
  );
  {
    ...
    try {
      assert.equal(
        entry("osm-japan").isAvailable([TOKYO]),
        false,
        "declines in Japan with no graph file",
      );
    } finally {
    ...
    if (hadRealGraph) {
      assert.equal(
        entry("osm-japan").isAvailable([TOKYO]),
        true,
        "available in Japan once the graph file is back",
      );
    }
  }
  ...
  await withEnv({ OSRM_FOOT_URL: undefined, OSRM_CAR_URL: undefined }, () => {
    assert.equal(
      entry("osrm").isAvailable([TOKYO]),
      false,
      "unavailable with neither URL set",
    );
  });
  await withEnv(
    { OSRM_FOOT_URL: "http://localhost:5002", OSRM_CAR_URL: undefined },
    () => {
      assert.equal(
        entry("osrm").isAvailable([TOKYO]),
        false,
        "unavailable with only the foot URL set",
      );
    },
  );
  await withEnv(
    ...
    () => {
      assert.equal(
        entry("osrm").isAvailable([TOKYO]),
        true,
        "available once both URLs are set",
      );
    },
  );
  ...
  await withEnv({ GOOGLE_MAPS_API_KEY: undefined }, () => {
    assert.equal(
      entry("google").isAvailable([TOKYO]),
      false,
      "unavailable with no key",
    );
  });
  await withEnv({ GOOGLE_MAPS_API_KEY: "test-key" }, () => {
    assert.equal(
      entry("google").isAvailable([PARIS]),
      true,
      "available anywhere once a key is set — not Japan-gated",
    );
  });
  ...
  assert.equal(
    entry("haversine").terminal,
    true,
    "exactly one entry is marked terminal",
  );
  ...
  //    back to a throw believing this to be a regression. ──
  {
    ...
    try {
      ...
      const matrix = await buildTravelMatrix([TOKYO, TOKYO], {
        kinds: ["rail"],
      });
      assert.equal(
        matrix[0][1].answeredBy,
        "haversine",
        "falls through to haversine instead of throwing",
      );
    } finally {
      if (hadRealGraph) fs.renameSync(backupPath, DEFAULT_GRAPH_PATH);
    }
  ...
  await withEnv(
    ...
    async () => {
      ...
      const matrix = await buildTravelMatrix(points, {
        kinds: ["rail", "bus", "walking", "driving"],
      });
  ...
  const sample = (answeredBy: ProviderId) =>
    makeTravelCost(1000, 100, "routingService", answeredBy);
  assert.equal(
    isPersistable(sample("google")),
    false,
    "a Google-derived cost is never persistable",
  );
