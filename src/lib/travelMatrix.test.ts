...
const cost = (id: ProviderId) => makeTravelCost(1000, 100, "routingService", id);
...
// ── First-capable-wins per cell; a declining cell falls through to the next entry ──
{
  const entryA = fakeEntry("osrm", ["walking"], (points) => {
    ...
    const m: MatrixCell[][] = Array.from({ length: n }, () => new Array(n).fill(null));
    if (n >= 2) m[0][1] = cost("osrm"); // answers exactly one cell, declines the rest
    return m;
  });
  const entryB = fakeEntry("haversine", ["walking"], (points) => {
    const n = points.length;
    return Array.from({ length: n }, () => new Array(n).fill(cost("haversine")));
  });

  const matrix = await composeTravelMatrix([entryA, entryB], [P0, P1], { kinds: ["walking"] });

  assert.equal(matrix[0][1].answeredBy, "osrm", "the first entry's answer wins and is never overwritten");
  assert.equal(matrix[1][0].answeredBy, "haversine", "a cell entryA declined falls through to entryB");
...
// ── An entry whose kinds don't intersect the request is never invoked at all ──
{
  ...
  const terminal = fakeEntry("haversine", ["rail", "bus", "walking", "driving", "bicycle", "other"], (points) => {
    const n = points.length;
    return Array.from({ length: n }, () => new Array(n).fill(cost("haversine")));
  });

  const matrix = await composeTravelMatrix([busOnly, terminal], [P0, P1], { kinds: ["walking"] });

  assert.equal(busOnly.calls.length, 0, "an entry with no kind overlap is skipped, not called with an empty kind list");
...
// ── Each entry receives exactly kinds ∩ request.kinds, in the entry's own kind order ──
{
  ...
  await composeTravelMatrix([spy], [P0, P1], { kinds: ["rail", "bus", "walking"] });

  assert.deepEqual(spy.calls[0].kinds, ["walking"], "driving is dropped — it's not in the request");
}
...
// ── No entry is called after the matrix is already complete (early exit) ──
{
  ...
  await composeTravelMatrix([fillsEverything, neverReached], [P0, P1], { kinds: ["walking"] });

  assert.equal(neverReached.calls.length, 0, "nothing left to do, so the next entry is never invoked");
}
...
//    ADR-0024 §7's "exposure bounded by composition," not the full N² ──
{
  ...
  const matrix = await composeTravelMatrix([almostEverything, spy], [P0, P1, P2, P3], { kinds: ["walking"] });

  assert.equal(spy.calls.length, 1);
  assert.equal(spy.calls[0].points.length, 2, "only the two still-unresolved points are handed to the next entry");
  ...
  assert.equal(matrix[0][3].answeredBy, "osrm", "cells resolved by the first entry are untouched");
}
...
// ── No terminal entry, matrix stays incomplete: throws rather than returning a hole ──
{
  ...
  await assert.rejects(
    () => composeTravelMatrix([declinesEverything], [P0, P1], { kinds: ["walking"] }),
