...
function fakeVroomSolve(body: VroomRequest): VroomSolution {
  const routes = body.vehicles.map((v) => ({
    vehicle: v.id,
    steps: [{ type: "start" }, { type: "end" }] as VroomStep[],
  }));
  body.jobs.forEach((job, i) => {
    const route = routes[i % routes.length];
    route.steps.splice(route.steps.length - 1, 0, {
      type: "job",
      id: job.id,
...
function mockFetch(
  handlers: {
    vroom?: (body: VroomRequest) => VroomSolution;
    osrmTable?: () => unknown;
  },
  calls: { vroom: number; osrm: number },
) {
  global.fetch = (async (url: string, init?: RequestInit) => {
    if (url === process.env.VROOM_URL) {
      calls.vroom++;
      const body = JSON.parse(init!.body as string) as VroomRequest;
      const solution = handlers.vroom
        ? handlers.vroom(body)
        : fakeVroomSolve(body);
...
async function main() {
  ...
  await withEnv({ VROOM_URL: undefined }, async () => {
    await assert.rejects(
      () =>
        solve({
          locations: [{ id: "a", lat: 35, lng: 139, kind: "activity" }],
          numDays: 1,
        }),
  ...
  await withEnv({ VROOM_URL: "http://localhost:8080" }, async () => {
    ...
    assert.deepEqual(itinerary.days[0].locationIds.slice().sort(), [
      "a1",
      "a2",
    ]);
  ...
  await withEnv({ VROOM_URL: "http://localhost:8080" }, async () => {
    ...
    assert.equal(
      capturedJobCount,
      1,
      "the ungeocoded Activity never became a job",
    );
    assert.deepEqual(
      itinerary.unplaced.map((u) => u.locationId),
      ["pending"],
    );
  ...
  await withEnv(
    ...
    async () => {
      ...
      mockFetch(
        {
          osrmTable: () => ({
            ...
            sources: [
              { location: [139.0, 35.0], distance: 0.5 },
              { location: [139.01, 35.01], distance: 0.5 },
            ],
            destinations: [
              { location: [139.0, 35.0], distance: 0.5 },
              { location: [139.01, 35.01], distance: 0.5 },
      ...
      assert.equal(
        calls.osrm,
        1,
        "the travel matrix is fetched exactly once per solve() call",
      );
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = (async (url: string, init?: RequestInit) => {
        if (url === process.env.VROOM_URL) {
          ...
          return {
            ok: true,
            status: 200,
            json: async () => fakeVroomSolve(body),
          } as Response;
        }
      ...
      await solve({
        ...
        kinds: ["walking"],
        journeyRoadKinds: [
          { locationAId: "a1", locationBId: "a2", kind: "driving" },
        ],
      });
      ...
      assert.equal(
        calls.foot,
        1,
        "one full-matrix call at the Trip default (foot)",
      );
      assert.equal(
        calls.car,
        1,
        "one override call at the chosen kind (car), scoped to just this pair",
      );
      assert.equal(calls.vroom, 1);
      ...
      assert.deepEqual(
        ...
        "the chosen cell used the car-profile duration, not the foot default",
      );
    },
  );
  ...
  await withEnv(
    ...
    async () => {
      ...
      await solve({
        ...
        kinds: ["walking"],
        journeyRoadKinds: [
          { locationAId: "a1", locationBId: "a2", kind: "walking" },
        ],
      });
      assert.equal(
        calls.osrm,
        1,
        "a choice matching the Trip default triggers no override call",
      );
    },
  );
  ...
  await withEnv(
    ...
    async () => {
      ...
      const itinerary = await solve({
        ...
        kinds: ["walking"],
        journeyRoadKinds: [
          { locationAId: "a1", locationBId: "gone", kind: "driving" },
        ],
      });
      assert.equal(
        calls.osrm,
        1,
        "the missing-Location choice is skipped, not attempted",
      );
      assert.equal(
        itinerary.days.flatMap((d) => d.locationIds).length,
        2,
  ...
  (globalThis as unknown as { _drizzle?: Promise<typeof db> })._drizzle =
    Promise.resolve(db);

  await withEnv(
    {
      VROOM_URL: "http://localhost:8080",
      OSRM_FOOT_URL: undefined,
      OSRM_CAR_URL: undefined,
    },
    async () => {
      const calls = { vroom: 0, osrm: 0 };
      ...
      await setLodgingDates(trip.id, id("H"), {
        checkInDate: "2026-06-24",
        checkOutDate: "2026-06-27",
      });
      const W = (
        await createLocation(trip.id, {
          name: "W (excluded)",
          lat: 35.2,
          lng: 139.2,
        })
      ...
      assert.deepEqual(
        [...placed].sort(),
        [id("X"), id("Y"), id("Z")].sort(),
        "all three activities are placed",
      );
      ...
      for (const p of after.trip.placements) {
        assert.ok(
          tripDateSet.has(p.date),
          `placement date ${p.date} is within the trip`,
        );
        assert.ok(
          isActivity(after.trip.locations.find((l) => l.id === p.locationId)!),
          "only activities are placed",
        );
      }
      ...
      assert.equal(
        again.trip.placements.length,
        after.trip.placements.length,
        "re-optimize replaces, never appends",
      );
      assert.equal(
        (await getTripWithDetails(trip.id))!.placements.length,
        3,
        "exactly the three activities remain placed",
      );
    },
  );
  ...
  await withEnv(
    ...
    async () => {
      ...
      const placed = new Set(after.trip.placements.map((p) => p.locationId));
      assert.ok(
        !placed.has(id("Airport")),
        "the arrival is an Anchor, never a Placement",
      );
      assert.equal(
        capturedJobCount,
        1,
        "only the real Activity became a VROOM job — the airport never did",
      );
    },
  );
