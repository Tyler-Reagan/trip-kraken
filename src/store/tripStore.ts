...
export const useTripStore = create<TripStore>()((set, get) => {
  // Optimistic single-field Trip update, shared by every trip-level flag setter below
  // (setRoadProfile, setTransitCaveatDismissed, setHasJrPass, ...): update local state
  // immediately, then persist via PATCH. Generic over the field so a new flag never needs
  // another near-identical copy of this six-line shape.
  function setTripField<K extends keyof TripWithDetails>(field: K) {
    return async (value: TripWithDetails[K]) => {
      const tripId = get().tripId;
      const t = get().trip;
      if (!tripId || !t) return;
      set({ trip: { ...t, [field]: value } });
  ...
  return {
    ...
    setTransitCaveatDismissed: setTripField("transitCaveatDismissed"),
    ...
    setRoadProfile: setTripField("roadProfile"),
    ...
    // Not a `setTripField` — a Journey's chosen kind lives in `journeyRoadKinds`, a list keyed by
    // Location pair rather than a scalar Trip column, so the optimistic update replaces/removes
    // one entry instead of overwriting a single field. Canonicalizes the pair the same way the
    // server does (sorted ids) so the optimistic list matches what a reload would return.
    setJourneyRoadKind: async (fromLocationId, toLocationId, kind) => {
      const tripId = get().tripId;
      const t = get().trip;
      if (!tripId || !t) return;
      ...
      await fetch(`/api/trips/${tripId}/journey-kinds`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fromLocationId, toLocationId, kind }),
      });
    },
